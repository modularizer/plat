...
import { connectFirstDiscovery, createClientSideServerMQTTWebRTCPeerSession, discoverClientSideServers, } from './mqtt-webrtc';
export function createClientSideServerMultiWorkerPool(options = {}) {
...
async function createWorkerPoolSession(address, options) {
    ...
    async function connectAdditionalWorkers(result) {
        ...
        for (const candidate of newCandidates) {
            ...
            if (!needActive && !needStandby)
                break;
            try {
                const session = await createClientSideServerMQTTWebRTCPeerSession(address, options, candidate.instanceId);
                if (closed) {
                    await session.close?.();
                    return;
                }
    ...
    function createWorkerFromSession(session, isActive) {
        return {
    ...
    function createWorkerFromCandidate(session, candidate) {
    ...
    function resolveWeight(candidate, opts) {
    ...
    function setupWorkerMessageForwarding(worker) {
    ...
    function promoteStandbyIfNeeded() {
    ...
    async function runHealthChecks() {
        const now = Date.now();
        for (const worker of workers) {
            if (worker.status !== 'active' && worker.status !== 'standby')
                continue;
            ...
            try {
                await worker.session.send({ platcss: 'ping', ts: now });
            }
            catch {
                worker.status = 'failed';
                promoteStandbyIfNeeded();
