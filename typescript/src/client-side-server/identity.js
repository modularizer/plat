...
export function loadAllTrustedClientSideServerRecordsForName(knownHosts, serverName) {
    if (!knownHosts)
        return [];
    const entry = knownHosts[serverName];
    if (!entry)
...
export function isTrustedPublicKeyForServer(knownHosts, serverName, publicKeyJwk) {
...
export async function generateClientSideServerEncryptionKeyPair(options = {}) {
    const subtle = await resolveSubtle();
    const keyPair = await subtle.generateKey({
        name: 'X25519',
...
export async function createClientSideServerEncryptionKeyId(publicKeyJwk) {
...
export async function getClientSideServerEncryptionPublicKeyFingerprint(publicKeyJwk) {
    const subtle = await resolveSubtle();
    const digest = await subtle.digest('SHA-256', toBufferSource(encodeUtf8(stableStringify(publicKeyJwk))));
    return base64UrlEncode(new Uint8Array(digest));
}
...
export async function createSignedClientSideServerAuthorityRecordV2(authorityKeyPair, input) {
    const payload = {
...
export async function verifySignedClientSideServerAuthorityRecordV2(record, authorityPublicKeyJwk) {
    const { signature, ...payload } = record;
    if (payload.protocol !== 'plat-css-authority-v2')
        return false;
    return verifyClientSideServerChallenge(authorityPublicKeyJwk, stableStringify(payload), signature);
}
export async function verifyAnySignedClientSideServerAuthorityRecord(record, authorityPublicKeyJwk) {
...
export async function toClientSideServerEncryptionPublicIdentity(keyPair) {
    return {
        algorithm: keyPair.algorithm,
        publicKeyJwk: keyPair.publicKeyJwk,
        keyId: keyPair.keyId,
...
export function saveClientSideServerEncryptionKeyPair(keyPair, options = {}) {
    const storage = resolveStorage(options.storage);
    if (!storage)
        throw new Error('No storage available to save a client-side server encryption key pair');
    storage.setItem(options.storageKey ?? 'plat-css:enc-keypair', JSON.stringify(keyPair));
}
export function loadClientSideServerEncryptionKeyPair(options = {}) {
    const storage = resolveStorage(options.storage);
    if (!storage)
        return null;
...
export async function getOrCreateClientSideServerEncryptionKeyPair(options = {}) {
...
export async function trustClientSideServerFromAuthorityRecord(record, authorityPublicKeyJwk, options = {}) {
    ...
    const trusted = isClientSideServerSignedAuthorityRecordV2(record)
        ? {
...
export function isClientSideServerTrustedServerRecordV2(value) {
    return Boolean(value
        && typeof value === 'object'
        && 'signingPublicKeyJwk' in value
        && 'encryptionPublicKeyJwk' in value);
}
export function isClientSideServerSignedAuthorityRecordV2(value) {
    return Boolean(value
        && typeof value === 'object'
        && value.protocol === 'plat-css-authority-v2'
        && 'signingPublicKeyJwk' in value
        && 'encryptionPublicKeyJwk' in value);
}
export async function trustedClientSideServerRecordToResolvedIdentityBundle(record) {
...
function isResolvedIdentityBundle(value) {
    return Boolean(value
        && typeof value === 'object'
        && 'signing' in value
        && 'encryption' in value);
