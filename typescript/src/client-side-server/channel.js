...
const BINARY_CHUNK_FRAME_MAGIC = 0x50424631;
...
function encodeBinaryChunkFrame(id, offset, bytes) {
...
function decodeBinaryChunkFrame(value) {
    const framed = toUint8Array(value);
    if (!framed || framed.byteLength < BINARY_CHUNK_FRAME_HEADER_BYTES)
        return null;
    const view = new DataView(framed.buffer, framed.byteOffset, framed.byteLength);
    if (view.getUint32(0) !== BINARY_CHUNK_FRAME_MAGIC)
        return null;
    if (view.getUint8(4) !== BINARY_CHUNK_FRAME_VERSION)
        return null;
    ...
    if (payloadStart > framed.byteLength)
        return null;
...
function createAssembler() {
    ...
    const push = (id, offset, bytes) => {
        ...
        if (assembly.receivedBytes < assembly.meta.byteLength)
            return null;
