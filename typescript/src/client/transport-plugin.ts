...
export type OpenAPIClientTransportOutcome =
  | OpenAPIClientTransportResult
  | OpenAPIClientTransportFailure

export interface OpenAPIClientTransportPlugin<TConnection = unknown> {
  name: string
  canHandle(request: { baseUrl: string; transportMode: string }): boolean
...
export async function executeClientTransportPlugin<TConnection>(
  plugin: OpenAPIClientTransportPlugin<TConnection>,
  request: OpenAPIClientTransportRequest,
): Promise<unknown> {
  const connection = plugin.connect ? await plugin.connect(request) : (undefined as TConnection)
  try {
    if (plugin.onConnect) {
    ...
    if (plugin.getUpdate) {
      while (true) {
        ...
        await request.onEvent?.({
          id: update.id,
          event: update.event as PLATRPCEventKind,
          data: update.data,
        })
        if (plugin.onUpdate) {
